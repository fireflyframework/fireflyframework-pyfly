...
```python
from pyfly.client.protocols import GrpcClientBuilder

# Generated by `python -m grpc_tools.protoc ...`
from myapp.proto import greeter_pb2_grpc, greeter_pb2

channel = (
    GrpcClientBuilder()
    .with_target("greeter-service:50051")
    .build()  # GrpcClientBuilder has no build() — call channel() directly
)
```

Wait — `GrpcClientBuilder` has no `build()` method. Call `channel()` to
materialise the connection:
